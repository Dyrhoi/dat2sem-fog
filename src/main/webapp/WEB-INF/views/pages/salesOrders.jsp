...
<div class="container" id="orderInfo" data-evenOffer="${Order.evenOffer}"
    ...
    <div class="row">
        <div class="col-8">
            ...
            <button class="btn btn-warning" type="button" onclick="location.href='${pageContext.request.contextPath}/sales/orders/${requestScope.order.uuid}/#'">
            ...
            <c:choose>
                <c:when test="${requestScope.order.shed.length != null}">
                    <c:out value="${requestScope.order.shed.length}"/> X <c:out value="${requestScope.order.shed.width}"/>
                </c:when>
                <c:otherwise>
                    <p>N/A</p>
                </c:otherwise>
            ...
            <form method="post" action="${pageContext.request.contextPath}/sales/orders/">
                <div class="d-flex justify-content-between">
                    <div class="input-group-append">
                        ...
                        <span class="input-group-text" id="profitPercentSpan"><c:out value="${requestScope.order.offer}"/></span>
                    </div>
                    <div class="text-right">
                        <button class="btn btn-outline-success pull-right" type="submit" name="order-offer">Send tilbud</button>
                    </div>
                </div>
            </form>
        </div>
        <div class="bg-light col-4 float-right" id="customerInfo">
            ...
            <p>Kunde: <c:out value="${requestScope.order.customer.firstname}"/> <c:out value="${requestScope.order.customer.lastname}"/></p>
